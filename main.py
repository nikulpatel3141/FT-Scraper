...
FT_ENDPOINT = "https://markets-data-api-proxy.ft.com/research/webservices/securities/v1/quotes"
...
def query_ft_mkt_data(symbols : list) -> dict:
    ...
    return {sym: quote['quote'] for quote in data for sym in symbols if quote['symbolInput']==sym}


def main():    
    ft_data = query_ft_mkt_data(list(FT_SYMBOLS.values()))
    daily_ret = {disp_sym: ft_data[ft_sym]['change1DayPercent'] for disp_sym, ft_sym in FT_SYMBOLS.items()}
